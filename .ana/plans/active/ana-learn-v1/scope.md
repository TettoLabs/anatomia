...
**Very large active set (200+ findings).** Learn shouldn't review all 200 in one session. The triage should focus: accept-action findings first (quick wins), then risk-severity findings (highest value), then promote candidates. Cap at ~30 findings per session. "Reviewed 30 of 200 active findings. Run Learn again for the next batch."
