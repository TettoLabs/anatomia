...
**Verify detected commands.** The Test, Build, and Lint lines above are inferred from your package.json. In monorepos they may not target the right package. Override with `ana config set commands.test "your-command"` if needed.
</Callout>
...
If Verify finds issues, it produces a FAIL report. Run `claude --agent ana-build` to fix, then `claude --agent ana-verify` again. This is normal — <DocsStat value="rejections" /> of Anatomia's own <DocsStat value="proofCount" /> proofs had rejection cycles.
